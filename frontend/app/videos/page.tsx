  ...
  const VideoList = () => {
    ...
    return (
      ...
      <div className="flex justify-center items-center gap-2 mt-8 text-white">
        <Link href={`?page=${currentPage - 1}`} className={`px-3 py-1 border rounded ${currentPage === 1 ? 'pointer-events-none opacity-50' : ''}`}>
          Previous
        </Link>
        {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
          <Link
            key={page}
            href={`?page=${page}`}
            className={`px-3 py-1 border rounded ${currentPage === page ? 'bg-football-accent text-football-dark' : ''}`}
          >
            {page}
          </Link>
        ))}
        <Link href={`?page=${currentPage + 1}`} className={`px-3 py-1 border rounded ${currentPage === totalPages ? 'pointer-events-none opacity-50' : ''}`}>
          Next
        </Link>
      </div>
      </>
